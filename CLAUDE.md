...
| IDH Mutation Classifier (MONAI DenseNet121 3D, TCGA-LGG, GT-mask ROI crop) | AUC | **Case 1.0000** (test, 10 cases — perfect on this split, including the hard WT case TCGA-CS-6669 the 2D model got confidently wrong). Val AUC 1.0 stable from epoch 17. 3D context + bbox crop + balanced sampler. Caveat: eval uses GT mask for ROI — realistic inference would use predicted mask from SegResNet. |
