...
| Molecular IDH Classifier (RNA-seq + methylation multi-omics) | AUC | Pooled 5-fold CV best **0.993 ± 0.007** (MLP, early concat); source-holdout best AUC: LGG→GBM **0.985** (MLP), GBM→LGG **0.977** (Logistic). GBM-only minority AUPRC best **0.9502** (per-modality LightGBM, late fusion via mean of probs — beats RNA-seq-only LightGBM 0.9469 and early-concat LightGBM 0.9425). Reproduce via `scripts/exp_late_fusion_idh.py`. |
