...
| IDH Mutation Classifier (MONAI DenseNet121 3D, TCGA-LGG, GT-mask ROI crop) | AUC | **Case 1.0000** (test, 10 cases) when ROI cropped from GT mask. With **predicted mask** ROI (true E2E via `infer-monai`): Test accuracy 0.90 (9/10 correct), Test AUC 0.50 — the one wrong case (TCGA-CS-6669, hard WT) is confidently wrong, dragging AUC despite high accuracy. Train/inference distribution gap (classifier saw only GT-mask crops during training) is the next thing to address — train with bbox jitter / mix GT+pred masks. |
