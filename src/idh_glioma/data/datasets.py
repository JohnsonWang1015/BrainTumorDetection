...
def _resize_slice(t: torch.Tensor, size: int) -> torch.Tensor:
    """Resize a (C, H, W) tensor to (C, size, size) using bilinear interpolation."""
...
class BraTSSliceSegmentationDataset(Dataset[tuple[torch.Tensor, torch.Tensor]]):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        target_t = _resize_slice(torch.from_numpy(target), self.img_size)
