"""5-fold stratified CV for the 3D MONAI IDH classifier.

Pools every labelled case from artifacts/manifest.json (across train / val /
test), splits into 5 stratified folds by IDH label, and trains a fresh
DenseNet121 3D per fold via 'uv run train-idh-monai' subprocess. Reports
mean +/- std of best smoothed val AUC -- a more robust estimate than the
single-split numbers for the MONAI 3D pipeline.

Resumability: skips folds whose checkpoint already exists. Delete
artifacts/cv_monai/ to start over.

Usage::

    uv run python scripts/cv_idh_monai.py --output-dir artifacts/cv_monai
"""
...
def main():
    ...
    p.add_argument("--output-dir", type=Path, default=Path("artifacts/cv_monai"))
    ...
    print(f"Total labelled cases: {len(all_records)} (WT={int((labels==0).sum())}, Mutant={int((labels==1).sum())})")
    ...
    for fold_i, (train_idx, val_idx) in enumerate(skf.split(all_records, labels)):
        ...
        print(f"  train: {len(train_recs)} (WT={train_lbls.count(0)}, Mut={train_lbls.count(1)})")
        print(f"  val:   {len(val_recs)} (WT={val_lbls.count(0)}, Mut={val_lbls.count(1)})")

        manifest_path = args.output_dir / f"cv_fold_{fold_i}.json"
        manifest_path.write_text(json.dumps({"train": train_recs, "val": val_recs, "test": []}))
        ...
        if ckpt_path.exists():
            ...
            cmd = [
                ...
                "--jitter-expand-max", str(args.jitter_expand_max),
                "--jitter-shift-max", str(args.jitter_shift_max),
        ...
        smoothed = float(state.get("smoothed_auc", float("nan")))
        raw = float(state.get("val_auc", float("nan")))
        epoch = int(state.get("epoch", -1))
        print(f"  best: smoothed AUC {smoothed:.4f} (raw {raw:.4f}) @ epoch {epoch}")
        fold_results.append({
            "fold": fold_i, "smoothed_auc": smoothed, "val_auc": raw, "epoch": epoch,
    ...
    print(f"\n=== {args.n_folds}-fold CV summary (3D MONAI DenseNet + jitter) ===")
