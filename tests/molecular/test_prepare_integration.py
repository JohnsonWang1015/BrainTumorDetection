...
def _write_rnaseq(path: Path, tpm_value: float) -> None:
    ...
    body = "\n".join(
        [
            ...
            f"ENSG00000000457.14\tSCYL3\tprotein_coding\t0\t0\t0\t{(tpm_value/4):.1f}\t0\t0",
            f"ENSG00000000460.17\tC1orf112\tprotein_coding\t0\t0\t0\t{(tpm_value/5):.1f}\t0\t0",
        ]
    )
...
def test_prepare_main_end_to_end_with_mock_data(tmp_path: Path, monkeypatch) -> None:
    gbm_root = tmp_path / "tcga_gbm_downloads"
    data_root = gbm_root / "data"
    rnaseq_root = data_root / "rnaseq_counts"
    maf_root = data_root / "maf"
    output_dir = tmp_path / "artifacts" / "molecular"
    ...
    expression_path = output_dir / "expression_matrix.parquet"
    labels_path = output_dir / "idh_labels.parquet"
    manifest_path = output_dir / "cohort_manifest.json"
    panel_path = output_dir / "feature_panel.json"
