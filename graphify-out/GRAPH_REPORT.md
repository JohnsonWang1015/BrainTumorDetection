...
## Summary
- 631 nodes · 928 edges · 71 communities detected
- Extraction: 59% EXTRACTED · 41% INFERRED · 0% AMBIGUOUS · INFERRED: 378 edges (avg confidence: 0.5)
...
Cohesion: 0.07
Nodes (25): _augment_cls(), _augment_seg(), BraTSSliceClassificationDataset, _crop_roi(), load_nifti(), make_balanced_sampler(), Random spatial + intensity augmentations for classification (image only, C,H,W)., Build a WeightedRandomSampler that balances IDH=0 vs IDH=1 across an epoch. (+17 more)
...
Cohesion: 0.12
Nodes (35): best_slice(), ct_gallery_extended(), dice(), _equal_box(), idh_e2e_gallery(), idh_montage(), idh_roi_orbit_3d(), _largest_cc() (+27 more)
...
Cohesion: 0.18
Nodes (14): build_manifest(), _collect_split_dir(), ImageRecord, main(), parse_args(), Build a train/val/test manifest for the Kaggle 4-class brain-tumor dataset.  Sou, _stratified_train_val_split(), _summarize() (+6 more)
...
Cohesion: 0.28
Nodes (11): _make_brats2021_case(), _make_tcga_case(), Unit tests for ``prepare_dataset.parse_case_folder``.  Covers two case-folder la, test_brats2021_case_id_is_dir_name(), test_brats2021_missing_modality_returns_none(), test_brats2021_seg_is_used_as_mask(), test_brats2021_t1ce_is_mapped_to_t1Gd(), test_tcga_case_id_and_date_unchanged() (+3 more)
...
Cohesion: 0.53
Nodes (5): _build_transforms(), main(), parse_args(), Generate native-space SegResNet 3D segmentation masks for chosen cases.  `eval-s, _records_to_monai()

### Community 32 - "Community 32"
Cohesion: 0.47
Nodes (5): download_cases(), list_cases(), main(), Download the BraTS 2021 mirror hosted on Hugging Face (no auth required).  Sourc, Download all files for the given cases in parallel via snapshot_download.      `
...
## Knowledge Gaps
- **62 isolated node(s):** `Unit tests for ``prepare_dataset.parse_case_folder``.  Covers two case-folder la`, `Unit tests for end-to-end ROI helpers used by the MONAI pipeline.`, `Tests for Phase 2 3D IDH ROI augmentation helpers.`, `Tests for end-to-end calibration config loading.`, `Pick the optimal IDH classification threshold on the val split.  Uses case-level` (+57 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **Thin community `Community 64`** (2 nodes): `__init__.py`, `Training entry points.`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 65`** (2 nodes): `test_manifest_adapters.py`, `test_to_legacy_manifest_record_keeps_training_fields()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 66`** (2 nodes): `test_eval.py`, `test_eval_main_all_modes_writes_outputs()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 67`** (2 nodes): `test_feature_select.py`, `test_select_features_top_k_and_prior_union_fold_aware()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 68`** (2 nodes): `test_train_eval_multimodal.py`, `test_train_eval_multimodal_end_to_end()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 69`** (2 nodes): `test_idh_cpg_panel.py`, `test_idh_cpg_panel_schema()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 70`** (1 nodes): `model.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `UNet2D` connect `Community 4` to `Community 6`?**
...
- **Are the 13 inferred relationships involving `load()` (e.g. with `seg_overlay()` and `idh_montage()`) actually correct?**
...
- **Are the 10 inferred relationships involving `dice()` (e.g. with `seg_overlay()` and `seg_gallery()`) actually correct?**
  _`dice()` has 10 INFERRED edges - model-reasoned connections that need verification._
